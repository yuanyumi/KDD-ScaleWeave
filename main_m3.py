"""M3 short-term forecasting entry point for ScaleWeave.

Cloned from main_m4.py with M4→M3 substitutions and B2 conservative defaults
(d_model=64, d_ff=128, n_heads=8, e_layers=3) since M3 has only 3003 series.
...
from data_provider.m3 import M3Meta
from models.ScaleWeave import ScaleWeave
from utils.losses import mape_loss, mase_loss, smape_loss
from utils.m3_summary import M3Summary
...
def build_parser():
    p = argparse.ArgumentParser(description='ScaleWeave M3 short-term forecasting')
    # data
    p.add_argument('--root_path', type=str, default='./datasets/m3/')
    p.add_argument('--data', type=str, default='m3')
    p.add_argument('--seasonal_patterns', type=str, required=True,
                   choices=M3Meta.seasonal_patterns)
...
def setup_args(args):
    ...
    args.frequency_map = M3Meta.frequency_map[args.seasonal_patterns]
    args.data_name = 'm3_' + args.seasonal_patterns
...
def maybe_summarize(out_dir, root_path):
    needed = {f'{s}_forecast.csv' for s in M3Meta.seasonal_patterns}
    present = set(os.listdir(out_dir))
    if needed.issubset(present):
        smapes, mapes = M3Summary(out_dir + '/', root_path).evaluate()
        print('smape:', smapes)
        print('mape :', mapes)
        return smapes, mapes
    missing = needed - present
    print(f'[M3Summary] waiting for subsets: {sorted(missing)}')
    return None
...
def main():
    ...
    out_dir = os.path.join('./m3_results',
