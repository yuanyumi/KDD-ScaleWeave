"""Tourism short-term forecasting entry point for ScaleWeave.

Cloned from main_m4.py with M4→M3 substitutions and B2 conservative defaults
(d_model=64, d_ff=128, n_heads=8, e_layers=3) since Tourism dataset.
...
from data_provider.tourism import TourismMeta
from models.ScaleWeave import ScaleWeave
from utils.losses import mape_loss, mase_loss, smape_loss
from utils.tourism_summary import TourismSummary
...
def build_parser():
    p = argparse.ArgumentParser(description='ScaleWeave Tourism short-term forecasting')
    # data
    p.add_argument('--root_path', type=str, default='./datasets/tourism/')
    p.add_argument('--data', type=str, default='tourism')
    p.add_argument('--seasonal_patterns', type=str, required=True,
                   choices=TourismMeta.seasonal_patterns)
    ...
    # misc
    p.add_argument('--fname', type=str, default='./checkpoints/tourism')
    p.add_argument('--wd_project', type=str, default='tourism')
    p.add_argument('--run_name', type=str, default='hsg_tourism')
...
def setup_args(args):
    ...
    args.frequency_map = TourismMeta.frequency_map[args.seasonal_patterns]
    args.data_name = 'tourism_' + args.seasonal_patterns
...
def maybe_summarize(out_dir, root_path):
    needed = {f'{s}_forecast.csv' for s in TourismMeta.seasonal_patterns}
    present = set(os.listdir(out_dir))
    if needed.issubset(present):
        smapes, mapes = TourismSummary(out_dir + '/', root_path).evaluate()
        print('smape:', smapes)
        print('mape :', mapes)
        return smapes, mapes
    missing = needed - present
    print(f'[TourismSummary] waiting for subsets: {sorted(missing)}')
    return None
...
def main():
    ...
    out_dir = os.path.join('./tourism_results',
