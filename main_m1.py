...
from data_provider.m1 import M1Meta
from models.ScaleWeave import ScaleWeave
from utils.losses import mape_loss, mase_loss, smape_loss
from utils.m1_summary import M1Summary
...
def build_parser():
    ...
    p.add_argument('--seasonal_patterns', type=str, required=True,
                   choices=M1Meta.seasonal_patterns)
    ...
    p.add_argument('--run_name', type=str, default='scaleweave_m1')
...
def setup_args(args):
    if args.pred_len == 0:
        args.pred_len = M1Meta.horizons_map[args.seasonal_patterns]
    if args.seq_len == 0:
        args.seq_len = 2 * args.pred_len
    if args.label_len == 0:
        args.label_len = args.pred_len
    args.frequency_map = M1Meta.frequency_map[args.seasonal_patterns]
...
def maybe_summarize(out_dir, root_path):
    needed = {f'{s}_forecast.csv' for s in M1Meta.seasonal_patterns}
    present = set(os.listdir(out_dir))
    if needed.issubset(present):
        smapes, mapes = M1Summary(out_dir + '/', root_path).evaluate()
        print('smape:', smapes)
        print('mape :', mapes)
        return smapes, mapes
    missing = needed - present
    print(f'[M1Summary] waiting for subsets: {sorted(missing)}')
    return None
...
def main():
    ...
    out_dir = os.path.join('./m1_results',
