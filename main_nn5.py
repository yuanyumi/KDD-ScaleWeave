"""NN5 short-term forecasting entry point for ScaleWeave.

Cloned from main_m4.py with M4→M3 substitutions and B2 conservative defaults
(d_model=64, d_ff=128, n_heads=8, e_layers=2) since NN5 dataset.
...
from data_provider.nn5 import NN5Meta
from models.ScaleWeave import ScaleWeave
from utils.losses import mape_loss, mase_loss, smape_loss
from utils.nn5_summary import NN5Summary
...
def build_parser():
    p = argparse.ArgumentParser(description='ScaleWeave NN5 short-term forecasting')
    # data
    p.add_argument('--root_path', type=str, default='./datasets/nn5/')
    p.add_argument('--data', type=str, default='nn5')
    p.add_argument('--seasonal_patterns', type=str, required=True,
                   choices=NN5Meta.seasonal_patterns)
    ...
    # misc
    p.add_argument('--fname', type=str, default='./checkpoints/nn5')
    p.add_argument('--wd_project', type=str, default='nn5')
    p.add_argument('--run_name', type=str, default='hsg_nn5')
    p.add_argument('--itr', type=int, default=1)
    p.add_argument('--exp_tag', type=str, default='', help='if set, output dir becomes ScaleWeave_<exp_tag>')
...
def setup_args(args):
    if args.pred_len == 0:
        args.pred_len = NN5Meta.horizons_map[args.seasonal_patterns]
    if args.seq_len == 0:
        args.seq_len = 2 * args.pred_len
    if args.label_len == 0:
        args.label_len = args.pred_len
    args.frequency_map = NN5Meta.frequency_map[args.seasonal_patterns]
    args.data_name = 'nn5_' + args.seasonal_patterns
...
def maybe_summarize(out_dir, root_path):
    needed = {f'{s}_forecast.csv' for s in NN5Meta.seasonal_patterns}
    present = set(os.listdir(out_dir))
    if needed.issubset(present):
        smapes, mapes = NN5Summary(out_dir + '/', root_path).evaluate()
        print('smape:', smapes)
        print('mape :', mapes)
        return smapes, mapes
    missing = needed - present
    print(f'[NN5Summary] waiting for subsets: {sorted(missing)}')
    return None
...
def main():
    ...
    out_dir = os.path.join('./nn5_results',
