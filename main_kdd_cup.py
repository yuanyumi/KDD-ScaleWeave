"""KDDCup short-term forecasting entry point for ScaleWeave.

Cloned from main_m4.py with M4→M3 substitutions and B2 conservative defaults
(d_model=64, d_ff=128, n_heads=8, e_layers=3) since KDDCup dataset.
...
from data_provider.kdd_cup import KDDCupMeta
from models.ScaleWeave import ScaleWeave
from utils.losses import mape_loss, mase_loss, smape_loss
from utils.kdd_cup_summary import KDDCupSummary
...
def build_parser():
    p = argparse.ArgumentParser(description='ScaleWeave KDDCup short-term forecasting')
    # data
    p.add_argument('--root_path', type=str, default='./datasets/kdd_cup/')
    p.add_argument('--data', type=str, default='kdd_cup')
    p.add_argument('--seasonal_patterns', type=str, required=True,
                   choices=KDDCupMeta.seasonal_patterns)
    ...
    # misc
    p.add_argument('--fname', type=str, default='./checkpoints/kdd_cup')
    p.add_argument('--wd_project', type=str, default='kdd_cup')
    p.add_argument('--run_name', type=str, default='hsg_kdd_cup')
    ...
    # DeformableTST-style optim knobs
...
def setup_args(args):
    if args.pred_len == 0:
        args.pred_len = KDDCupMeta.horizons_map[args.seasonal_patterns]
    if args.seq_len == 0:
        args.seq_len = 2 * args.pred_len
    if args.label_len == 0:
        args.label_len = args.pred_len
    args.frequency_map = KDDCupMeta.frequency_map[args.seasonal_patterns]
    args.data_name = 'kdd_cup_' + args.seasonal_patterns
...
def maybe_summarize(out_dir, root_path):
    needed = {f'{s}_forecast.csv' for s in KDDCupMeta.seasonal_patterns}
    present = set(os.listdir(out_dir))
    if needed.issubset(present):
        smapes, mapes = KDDCupSummary(out_dir + '/', root_path).evaluate()
        print('smape:', smapes)
        print('mape :', mapes)
        return smapes, mapes
    missing = needed - present
    print(f'[KDDCupSummary] waiting for subsets: {sorted(missing)}')
    return None
...
def main():
    ...
    if args.lradj == 'COS':
        ...
        if args.warmup_epochs > 0:
            warmup_sched = torch.optim.lr_scheduler.LinearLR(
                model_optim, start_factor=0.01, end_factor=1.0,
    ...
    out_dir = os.path.join('./kdd_cup_results',
    ...
    # Inline per-subset SMAPE — same pattern as main_m1.py / main_m3.py.
    trues_arr = np.stack([np.asarray(t, dtype=np.float32) for t in trues])
    denom = np.abs(preds) + np.abs(trues_arr)
    denom = np.where(denom == 0.0, 1.0, denom)
    subset_smape = float(np.mean(200 * np.abs(preds - trues_arr) / denom))
    print(f'subset_smape: {args.seasonal_patterns}={subset_smape:.4f}')
