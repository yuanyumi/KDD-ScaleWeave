...
def build_parser():
    ...
    p.add_argument('--source_pattern', type=str, default='')
    ...
    p.add_argument('--train_all', type=int, default=0,
                   help='If 1, train and val both span the full series minus the last pred_len.')
    p.add_argument('--token_len', type=int, default=0,
                   help='Per-step prediction length used at training. If 0, token_len=pred_len. '
                        'Set below test_pred_len to enable autoregressive rolling forecast at evaluation.')
...
def main():
    ...
    insample, _ = te_ds.last_insample_window()
    target_y = np.stack([ts[-args.test_pred_len:] for ts in te_ds.timeseries])

    x = torch.from_numpy(insample).float().unsqueeze(-1).to(device)
    model.eval()

    def rolling_forecast(model, ctx_x, token_len, target_len):
        """Autoregressive rolling: predict token_len chunks until total >= target_len over a sliding context window."""
        chunks = []
        ctx = ctx_x
        total = 0
